...
export const generateSeatLayout = (
  seatLayoutConfig?: { row: string; type: string; price: number; seatCount: number }[]
) => {
  const config = seatLayoutConfig && seatLayoutConfig.length > 0
    ? seatLayoutConfig
    : [
...
export const groupShowsByTheaterAndMovie = (shows:IShow[]):GroupedShow[]=>{
   ...
   shows.forEach((show)=>{
      ...
      const totalSeats = layout.reduce((acc:number, row:any) => acc + (row.seats?.length ?? 0), 0);
      const availableSeats = layout.reduce((acc:number, row:any) => acc + (row.seats?.filter((s:any) => s.status === "AVAILABLE").length ?? 0), 0);
